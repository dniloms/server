...
-- 1000 - 100 due to exact condition timing. -100 doesn't hurt us, and players don't have reminding ~50ms exhaustion. 200
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif itemId == strongManaPot then
		...
		if not doTargetCombatMana(0, target, 230, 330, CONST_ME_MAGIC_BLUE) then
			return false
		end
	...
	elseif itemId == greatManaPot then
		...
		if not doTargetCombatMana(0, target, 330, 430, CONST_ME_MAGIC_BLUE) then
			return false
		end
		player:addCondition(exhaust)
		target:say("Aaaah...", TALKTYPE_MONSTER_SAY)
